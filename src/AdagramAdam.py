...
class AdagramAdam(AdaGramPS, AdaGram):
    """
        ...
        with torch.no_grad():
            for group in self.param_groups:
                for param_idx, p in enumerate(group["params"]):
                    ...
                    ## Adam part - CORRECTED
                    # Get previous Adam state
                    m_prev = state['m_t']  # Fixed: use state variables
                    v_prev = state['v_t']  # Fixed: use state variables
                    
                    # Update Adam moments
                    m_t = self.beta1 * m_prev + (1 - self.beta1) * precond_grad
                    ...
                    param_vector.add_(adam_update, alpha=-1.0)
