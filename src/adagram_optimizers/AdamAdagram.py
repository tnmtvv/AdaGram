    ...
    with torch.no_grad():
        for group in self.param_groups:
            for param_idx, p in enumerate(group["params"]):
                ...
                # Initialize state if needed - MUST BE FIRST
                if len(state) == 0:
                ...
                ## Adam part - NOW state is guaranteed to be initialized
                m_prev = state['m_t']
                v_prev = state['v_t']
                
                # Update Adam moments
                m_t = self.beta1 * m_prev + (1 - self.beta1) * grad_vector
                v_t = self.beta2 * v_prev + (1 - self.beta2) * grad_vector.mul(grad_vector)

                # Bias correction
                m_t_hat = m_t / (1 - self.beta1**state["step_count"])
                v_t_hat = v_t / (1 - self.beta2**state["step_count"])

                # Adam preconditioned gradient
                adam_grad = m_t_hat / (v_t_hat.sqrt() + group["eps"])

                # Store updated Adam state
                state['m_t'] = m_t
                state['v_t'] = v_t

                # AdaGram preconditioning on Adam gradient
                g_bar = self.update_grad_vector(state, adam_grad)
                ...
                # Apply final update to parameters
